...
# Remove default handler and re-add with unified guide_cortex format
logger.remove()
...
from guide_cortex import __logo__, __version__
...
from guide_cortex.cli.stream import StreamRenderer, ThinkingSpinner
from guide_cortex.config.paths import get_workspace_path, is_default_workspace
from guide_cortex.config.schema import Config
from guide_cortex.utils.helpers import sync_workspace_templates
from guide_cortex.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} guide_cortex - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from guide_cortex.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} guide_cortex[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} guide_cortex[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} guide_cortex v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """guide_cortex - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize guide_cortex configuration and workspace."""
    from guide_cortex.config.loader import get_config_path, load_config, save_config, set_config_path
    from guide_cortex.config.schema import Config
    ...
    if wizard:
        from guide_cortex.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'guide_cortex onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'guide_cortex agent -m "Hello!"'
    gateway_cmd = "guide_cortex gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} guide_cortex is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/guide_cortex#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from guide_cortex.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from guide_cortex.config.loader import load_config, load_dotenv_files, resolve_config_env_vars, set_config_path

    for env_path in load_dotenv_files():
        console.print(f"[dim]Loaded env: {env_path}[/dim]")
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from guide_cortex.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from guide_cortex.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show guide_cortex runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'guide_cortex-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from guide_cortex.api.server import create_app
    from guide_cortex.bus.queue import MessageBus
    from guide_cortex.session.manager import SessionManager

    if verbose:
        logger.enable("guide_cortex")
    else:
        logger.disable("guide_cortex")
    ...
    try:
        agent_loop = AgentLoop.from_config(
            ...
            image_generation_provider_configs={
                ...
                "volcengine": runtime_config.providers.volcengine,
            },
        )
...
def gateway(
    ...
    """Start the guide_cortex gateway."""
    if verbose:
...
def _run_gateway(
    ...
    *,
    ...
    from guide_cortex.agent.tools.cron import CronTool
    from guide_cortex.agent.tools.message import MessageTool
    from guide_cortex.bus.queue import MessageBus
    from guide_cortex.channels.manager import ChannelManager
    from guide_cortex.channels.websocket import publish_runtime_model_update
    from guide_cortex.cron.service import CronService
    from guide_cortex.cron.types import CronJob
    from guide_cortex.heartbeat.service import HeartbeatService
    from guide_cortex.providers.factory import build_provider_snapshot, load_provider_snapshot
    from guide_cortex.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting guide_cortex gateway version {__version__} on port {port}...")
    ...
    from guide_cortex.agent.loop import UNIFIED_SESSION_KEY
    from guide_cortex.bus.events import OutboundMessage
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.name == "persona-ingest":
            ...
            try:
                ...
                logger.info("Persona ingest cron completed: {}", result)
            except Exception:
                logger.exception("Persona ingest cron job failed")
            return None

        if job.name == "persona-upgrade":
            from guide_cortex.ingest.service import IngestService
            from guide_cortex.persona.upgrade import run_pending_upgrades

            try:
                service = IngestService(config.workspace_path)
                done = await run_pending_upgrades(agent, service)
                logger.info("Persona upgrade cron completed: {}", done)
            except Exception:
                logger.exception("Persona upgrade cron job failed")
            return None

        from guide_cortex.utils.evaluator import evaluate_response
    ...
    from guide_cortex.cron.types import CronJob, CronPayload
    ...
    ingest_cfg = config.agents.defaults.persona_ingest
    if ingest_cfg.enabled:
        cron.register_system_job(CronJob(
            id="persona-ingest",
            name="persona-ingest",
            schedule=ingest_cfg.build_ingest_schedule(config.agents.defaults.timezone),
            payload=CronPayload(kind="system_event"),
        ))
        console.print(f"[green]✓[/green] Persona ingest: {ingest_cfg.describe_ingest_schedule()}")
        upgrade_schedule = ingest_cfg.build_upgrade_schedule(config.agents.defaults.timezone)
        if upgrade_schedule is not None:
            cron.register_system_job(CronJob(
                id="persona-upgrade",
                name="persona-upgrade",
                schedule=upgrade_schedule,
                payload=CronPayload(kind="system_event"),
            ))
            console.print(f"[green]✓[/green] Persona upgrade: cron {ingest_cfg.upgrade_cron}")

    ws_channel = channels.channels.get("websocket")
    if ws_channel is not None:
        ws_channel._agent_loop = agent
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show guide_cortex runtime logs during chat"),
):
    ...
    if message:
        ...
        from guide_cortex.bus.events import InboundMessage
...
def channels_status(
    ...
    """Show channel status."""
    from guide_cortex.channels.registry import discover_all
    from guide_cortex.config.loader import load_config, set_config_path
...
def plugins_list():
    ...
    from guide_cortex.channels.registry import discover_all, discover_channel_names
...
@app.command()
def status():
    """Show guide_cortex status."""
    from guide_cortex.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} guide_cortex Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from guide_cortex.providers.registry import PROVIDERS
...
def _logout_github_copilot() -> None:
    ...
    try:
        from guide_cortex.providers.github_copilot_provider import get_storage
    except ImportError:
...
def _login_github_copilot() -> None:
    try:
        from guide_cortex.providers.github_copilot_provider import login_github_copilot
