"""Interactive onboarding questionnaire for guide_cortex."""
...
from guide_cortex.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]guide_cortex[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from guide_cortex.config.schema to get
    ...
    from guide_cortex.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"guide_cortex.channels.{name}")
