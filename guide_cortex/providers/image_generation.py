...
from guide_cortex.utils.helpers import detect_image_mime

_OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/guide_cortex",
    "X-OpenRouter-Title": "guide_cortex",
...
def _volcengine_size(aspect_ratio: str | None, image_size: str | None) -> str:
    if image_size and "x" in image_size.lower():
        return image_size
    if image_size in {"1K", "2K", "4K"}:
        return image_size
    if aspect_ratio in _VOLCENGINE_SIZE_MAP:
        return _VOLCENGINE_SIZE_MAP[aspect_ratio]
    return "2K"


class VolcengineImageGenerationClient:
    """Async client for VolcEngine Ark /images/generations (Seedream)."""
    ...
    async def generate(
        ...
        *,
        ...
        if not self.api_key:
            raise ImageGenerationError(
                "VolcEngine API key is not configured. Set providers.volcengine.apiKey or ARK_API_KEY."
            )

        body: dict[str, Any] = {
            "model": model,
            "prompt": prompt,
            "size": _volcengine_size(aspect_ratio, image_size),
        ...
        try:
            ...
            detail = response.text[:500]
            raise ImageGenerationError(f"VolcEngine image generation failed: {detail}") from exc
        ...
        for item in data.get("data") or []:
            ...
            if isinstance(url_value, str) and url_value.startswith(("http://", "https://")):
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    images.append(await _download_image_data_url(client, url_value))

        if not images:
            provider_error = data.get("error") if isinstance(data, dict) else None
            if provider_error:
                raise ImageGenerationError(f"VolcEngine returned no images: {provider_error}")
            raise ImageGenerationError("VolcEngine returned no images for this request")
