"""WebSocket server channel: guide_cortex acts as a WebSocket server and serves connected clients."""
...
from guide_cortex.command.builtin import builtin_command_palette
from guide_cortex.config.paths import get_media_dir
from guide_cortex.config.schema import Base
from guide_cortex.utils.helpers import safe_filename
from guide_cortex.utils.media_decode import (
    FileSizeExceeded,
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      guide_cortex and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Nanobot-Auth") or headers.get("x-guide_cortex-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    async def _dispatch_http(self, connection: Any, request: WsRequest) -> Any:
        ...
        if got == "/api/guide/dashboard":
            return self._handle_guide_dashboard(request)

        if got.startswith("/api/guide/memories"):
            return self._handle_cortex_memories(request)

        if got.startswith("/api/guide/memory/inbox/add"):
            return self._handle_memory_inbox_add(request)

        if got.startswith("/api/guide/personas/detail"):
            return self._handle_persona_detail(request)

        if got.startswith("/api/guide/personas/ingest/add"):
            return await self._handle_persona_ingest_add(request)

        if got.startswith("/api/guide/personas/ingest/run"):
            return await self._handle_persona_ingest_run(request)

        if got.startswith("/api/guide/personas/ingest/status"):
            return self._handle_persona_ingest_status(request)

        if got.startswith("/api/guide/personas/upgrade/run"):
            return await self._handle_persona_upgrade_run(request)
    ...
    def _settings_payload(self, *, requires_restart: bool = False) -> dict[str, Any]:
        ...
        from guide_cortex.providers.registry import PROVIDERS, find_by_name
    ...
    def _handle_guide_dashboard(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.web.guide_dashboard import build_guide_dashboard

        query = _parse_query(request.path).get("q", [""])[0]
        return _http_json_response(build_guide_dashboard(query=query))

    def _handle_cortex_memories(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.web.guide_dashboard import build_guide_dashboard

        query = _parse_query(request.path).get("q", [""])[0]
        payload = build_guide_dashboard(query=query)
        return _http_json_response({"chunks": payload["memory"]["chunks"]})

    def _handle_memory_inbox_add(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        import base64

        from guide_cortex.config.loader import load_config
        from guide_cortex.web.memory_inbox import write_inbox_note

        query = _parse_query(request.path)
        title = (_query_first(query, "title") or "").strip()
        content_b64 = _query_first(query, "content_b64") or ""
        if not title:
            return _http_error(400, "title is required")
        if not content_b64:
            return _http_error(400, "content_b64 is required")
        try:
            content = base64.b64decode(content_b64).decode("utf-8")
        except (ValueError, UnicodeDecodeError):
            return _http_error(400, "invalid content_b64")
        if len(content) > 32_000:
            return _http_error(400, "content too large")

        config = load_config()
        try:
            path = write_inbox_note(config.workspace_path, title=title, content=content)
        except ValueError as exc:
            return _http_error(400, str(exc))
        return _http_json_response({"ok": True, "path": str(path)})

    def _handle_persona_detail(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.web.persona_ingest import persona_detail_with_ingest

        persona_id = _query_first(_parse_query(request.path), "id") or ""
        persona_id = persona_id.strip()
        if not persona_id:
            return _http_error(400, "id is required")
        try:
            payload = persona_detail_with_ingest(persona_id)
        except KeyError:
            return _http_error(404, "persona not found")
        return _http_json_response(payload)

    async def _handle_persona_ingest_add(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        import base64
        ...
        query = _parse_query(request.path)
        persona_id = (_query_first(query, "persona_id") or _query_first(query, "id") or "").strip()
        title = (_query_first(query, "title") or "采集内容").strip()
        content_b64 = _query_first(query, "content_b64") or ""
        ...
        if not url and not content.strip():
            return _http_error(400, "content or url is required")
        ...
        try:
            ...
            return _http_error(400, str(exc))
        return _http_json_response({"ok": True, "result": result.to_dict()})

    async def _handle_persona_ingest_run(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.config.loader import load_config
        from guide_cortex.ingest.service import IngestService

        persona_id = (_query_first(_parse_query(request.path), "id") or "").strip()
        config = load_config()
        service = IngestService(config.workspace_path)
        try:
            if persona_id:
                result = await service.run_persona(persona_id)
                payload = {"ok": True, "result": result.to_dict()}
            else:
                payload = {"ok": True, **await service.run_all()}
        except KeyError:
            return _http_error(404, "persona not found")
        return _http_json_response(payload)

    def _handle_persona_ingest_status(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.web.persona_ingest import ingest_status_payload

        persona_id = (_query_first(_parse_query(request.path), "id") or "").strip() or None
        return _http_json_response(ingest_status_payload(persona_id))

    async def _handle_persona_upgrade_run(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.config.loader import load_config
        from guide_cortex.ingest.service import IngestService
        from guide_cortex.persona.upgrade import run_persona_upgrade

        query = _parse_query(request.path)
        persona_id = (_query_first(query, "id") or "").strip()
        notes = (_query_first(query, "notes") or "").strip()
        if not persona_id:
            return _http_error(400, "id is required")

        agent = getattr(self, "_agent_loop", None)
        if agent is None:
            return _http_error(503, "agent not ready")

        config = load_config()
        service = IngestService(config.workspace_path)
        try:
            task = asyncio.create_task(run_persona_upgrade(agent, service, persona_id, notes=notes))
            task.add_done_callback(lambda t: t.exception() if not t.cancelled() else None)
        except KeyError:
            return _http_error(404, "persona not found")
        return _http_json_response({"ok": True, "started": True, "persona_id": persona_id})

    def _handle_settings_update(self, request: WsRequest) -> Response:
        if not self._check_api_token(request):
            return _http_error(401, "Unauthorized")
        from guide_cortex.config.loader import load_config, save_config
        from guide_cortex.providers.registry import find_by_name
    ...
    async def start(self) -> None:
        from guide_cortex.utils.logging_bridge import redirect_lib_logging
