...
def get_config_path() -> Path:
    ...
    new_path = Path.home() / ".guide_cortex" / "config.json"
    if new_path.exists():
        return new_path
    hyphen = Path.home() / ".guide-cortex" / "config.json"
    if hyphen.exists():
        return hyphen
    legacy = Path.home() / ".nuwa-cortex" / "config.json"
    if legacy.exists():
        return legacy
    return new_path
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from guide_cortex.security.network import configure_ssrf_whitelist
...
def load_dotenv_files() -> list[Path]:
    ...
    Existing environment variables are never overwritten. Files are loaded in
    order; later files only fill in keys that are still unset.
...
def _load_dotenv_file(path: Path) -> None:
    try:
        ...
        logger.warning("Could not read env file {}: {}", path, exc)
        return
...
def _migrate_config(data: dict) -> dict:
    ...
    if "openhuman" in data and "memory_tree" not in data:
        data["memory_tree"] = data.pop("openhuman")
