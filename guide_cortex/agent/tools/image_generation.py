...
from guide_cortex.agent.tools.base import Tool, tool_parameters
...
from guide_cortex.providers.image_generation import (
    AIHubMixImageGenerationClient,
    ImageGenerationError,
    OpenRouterImageGenerationClient,
    VolcengineImageGenerationClient,
)
from guide_cortex.utils.artifacts import (
    ArtifactError,
...
if TYPE_CHECKING:
    from guide_cortex.config.schema import ProviderConfig
...
class ImageGenerationTool(Tool):
    ...
    def _provider_client(
        ...
        if self.config.provider == "volcengine":
            return VolcengineImageGenerationClient(**kwargs)
        return None
    ...
    def _missing_api_key_error(self) -> str:
        ...
        if provider == "volcengine":
            return "Error: VolcEngine API key is not configured. Set providers.volcengine.apiKey or ARK_API_KEY."
    ...
    def _resolve_reference_image(self, value: str) -> str:
        ...
        if not any(_is_relative_to(resolved, root) for root in allowed_roots):
            raise ImageGenerationError(
                "reference_images must be inside the workspace or guide_cortex media directory"
            )
