...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../guide_cortex/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    },
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the guide_cortex ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the guide_cortex gateway;
