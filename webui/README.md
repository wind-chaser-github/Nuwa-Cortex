...
The browser front-end for the guide_cortex gateway. It is built with Vite + React 18 +
...
guide_cortex/web/dist/      build output served by the gateway
```

## Develop from source

### 1. Install guide_cortex from source
...
To use the webui from another device on the same network, set `host` to `"0.0.0.0"` and configure a `token` or `tokenIssueSecret` in `~/.guide_cortex/config.json`:
...
This writes the production assets to `../guide_cortex/web/dist`, which is the
directory served by `guide_cortex gateway` and bundled into the Python wheel.
